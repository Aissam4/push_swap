...
/*   Updated: 2021/12/12 15:30:49 by abarchil         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_find_smallest_number2(s_stack *a, int i)
{
	int	s_index;

	s_index = min_value(a->array + i, a->used_size);
	while (s_index != 0)
	{
		if (s_index <= 2)
			rotate((s_swap *)a);
...
void	sort_5(s_stack *a, s_stack *b)
{
	...
	i = 0;
	int size = a->used_size;
	while (a->array[i] && i < a->used_size)
	{
		if (a->array[a->used_size - 1] < a->array[a->used_size])
		{
			printf("%d",)
			//reverse_rotate((s_swap *)a);
		}
