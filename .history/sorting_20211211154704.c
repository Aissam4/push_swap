...
/*   Updated: 2021/12/11 15:47:04 by abarchil         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_find_smallest_number2(s_stack *a)
{
	int	s_index;

	s_index = min_value(a->array + 1, a->used_size);
	while (s_index != 0)
	{
		if (s_index <= 2)
			rotate((s_swap *)a);
			...
			reverse_rotate((s_swap*)a);
		s_index = min_value(a->array + 2, a->used_size);
	}
}
